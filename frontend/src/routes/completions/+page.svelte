<script lang="ts">
    ...
    async function refreshGames() {
        data.games = await GameAPI.completed();
    }
    ...
    {#each data.games as game}
        <div class="flex gap-4 group">
            ...
            <div class="flex flex-col flex-grow">
                ...
                <p class="text-surface0 line-clamp-3 group-hover:line-clamp-1">
                    {game.description}
                </p>
            ...
            {#if game.completed}
                <div
                    class="text-mauve flex flex-col items-center justify-center h-full gap-2 px-8"
                >
                    <Icon class="text-2xl" icon="bxs:calendar" />
                    <p class="text-center w-24 text-nowrap">
                        {formatDate(game.completed)}
                    </p>
                </div>
            {/if}

            <div
                class="text-peach flex flex-col items-center justify-center h-full gap-2 px-8"
            >
                <Icon class="text-2xl" icon="mingcute:time-fill" />
                <p class="text-center w-24 text-nowrap">
                    {game.playtime / 60} hours
                </p>
            </div>
        </div>
    {/each}
