<script lang="ts">
    ...
    async function refreshGames() {
        data.games = await GameAPI.backlog();
    }
</script>
...
<div class="w-full h-full flex flex-col p-4 gap-4">
        ...
        {#each data.games as game}
            ...
            >
                <img
                    class={twMerge(
                        'game h-48',
                        selectedGame?.id === game.id && 'outline outline-mauve',
                    )}
                    src={game.cover}
                    alt="Game Cover"
                />
    ...
    {#if selectedGame}
        <div class="mt-auto toolbar">
            ...
            <button on:click={() => moveGameModal.show()} class="toolbar-btn">
                <p>Move to list</p>
            ...
            <button on:click={() => deleteGameModal.show()} class="toolbar-btn">
                <Icon icon="mdi:trash" />
            </button>
        </div>
    {/if}
